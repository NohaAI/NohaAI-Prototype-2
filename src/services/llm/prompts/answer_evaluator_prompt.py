...
def make_prompt_from_template_Ritesh():
  eval_prompt_str = """
    ...
    Response:
  """
def make_prompt_from_template_current():
  eval_prompt_str = """
...
def make_prompt_from_template_Ritesh_2_7_2025():
  eval_prompt_str = """
    You are an expert interviewer with experience in evaluating responses to interview questions about topics such as Data Structures, Algorithms and Algorithmic complexity.
    ...
    1. Group the subcriteria recieved in the subcriteria payload above into seven consecutive groups of three each
    2. Please evaluate the answer as per the instructions provided in each subcriterion and associate the score in format as per the example shown below 
...
def make_prompt_from_template():
  eval_prompt_str = """
    ...
    IMPORTANT : Please ensure that the structure and length of the subcriteria remain the same and no new subcriterion are added because you would incur a penalty otherwise.
    YOU MUST RESPOND IN THE FOLLOWING LIST FORMAT:
    [
      "answer_evaluation",
      "rationale"
